...
if (isset($_POST['submit'])) {
    ...
    // ✅ Folder exist na kare to bana do
    $uploadDir = "../images/";
    if (!is_dir($uploadDir)) {
    ...
    // ✅ Check karo book pehle se exist to nahi
    $checkQuery = mysqli_prepare($conn, "SELECT * FROM books WHERE title = ?");
    ...
    if (mysqli_stmt_num_rows($checkQuery) > 0) {
        ...
        echo "❌ Ye book pehle se database me mojood hai!";
        exit();
    ...
    $stmt = mysqli_prepare($conn, "INSERT INTO books (title, author, price, image) VALUES (?, ?, ?, ?)");
